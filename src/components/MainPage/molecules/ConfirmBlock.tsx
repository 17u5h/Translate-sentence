...
const ConfirmBlock = () => {
  ...
  const { worksheetArray, dispatchWorksheetArray } = useWorkplaceStore(
    ({ worksheetArray, dispatchWorksheetArray }) => ({ worksheetArray, dispatchWorksheetArray })
  )
  const { englishPhraseArray } = usePhrasesStore(({ englishPhraseArray }) => ({
    englishPhraseArray
  }))
  const { playAgain, dispatchPlayAgain } = usePlayAgainStore(
    ({ playAgain, dispatchPlayAgain }) => ({
      playAgain,
      dispatchPlayAgain
    })
  )
